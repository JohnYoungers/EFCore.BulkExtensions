...
#pragma warning restore EF1001
        ...
        try
        {
            using var dbCommand = new SqlQueryBuilder().CreateCommand(); // Use a DbCommand to convert parameter values using ValueConverters to the correct type.
        ...
        catch (Exception ex) // Fix for BatchDelete with 'uint' param on Sqlite. TEST: RunBatchUint
        {
                ...
                ex.Message.StartsWith(npgsqlSpecParamMessage)) // Fix for BatchDelete with Contains on PostgreSQL
            {
                ...
                parameters = parameterValues.Where(pv => parameterNames.Contains(pv.Key)).Select(pv => new SqlQueryBuilder().CreateParameter("@" + pv.Key, pv.Value)).ToList();
            }
            else
            {
                throw;
